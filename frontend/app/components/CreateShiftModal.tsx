...
import { shiftsAPI, CreateShiftRequestData } from '@/lib/shifts-api';
...
export default function CreateShiftModal({
  ...
  useEffect(() => {
    if (defaultStartTime) {
      setFormData(prev => ({ ...prev, start_time: defaultStartTime }));
    }
  }, [defaultStartTime]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data: CreateShiftRequestData = {
      ...
      await shiftsAPI.createRequest(data);
      onSuccess();
      onClose();
      resetForm();
    } catch (err: any) {
      setError(err.response?.data?.detail || err.response?.data?.error || 'Failed to create shift request');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full relative z-[9999]">
          <form onSubmit={handleSubmit}>
            <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Request Shift</h3>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder="Any additional information about this shift request..."
                    className="w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 text-gray-900 bg-white focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                  />
                </div>
              </div>
